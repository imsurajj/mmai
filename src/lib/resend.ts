...
export async function sendEmail({ to, subject, html, text, from = FROM_EMAIL }: SendEmailOptions) {
  const apiKey =
    process.env.EXPO_PUBLIC_RESEND_API_KEY ||
    process.env.NEXT_PUBLIC_RESEND_API_KEY ||
    process.env.RESEND_API_KEY ||
  ...
  try {
    ...
    if (!response.ok) {
      if (data?.message?.includes('only send testing emails to your own email address')) {
        ...
        try {
          ...
          if (fallbackRes.ok) {
            console.log('[Resend Success] Email delivered to verified account:', ownerEmail, fallbackData);
            return {
              success: true,
