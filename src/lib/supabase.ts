 ...
 */
export async function loginPatientWithSecretKey(secretCode: string): Promise<PatientSession> {
  ...
  await safeStorage.setItem(PATIENT_SESSION_STORAGE_KEY, JSON.stringify(session));
...
export async function requestPatientLoginOtp(secretCode: string) {
  ...
  if (error) {
    throw new Error(error.message || 'Failed to request login verification code.');
  }
  ...
  if (data.patient_email && data.otp) {
    try {
      ...
      console.warn('[Resend Warning] Failed to dispatch login OTP email:', emailErr);
    }
  }
...
export async function createPatient({
  ...
  const { data, error } = await supabase.rpc('create_patient_with_access_code', {
